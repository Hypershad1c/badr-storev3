...
// Define a common return type for the server actions to resolve the Union error
...
export function UsersTable({ users }: { users: UserRow[] }) {
  ...
  const handleBan = async (id: string, banned: boolean) => {
    // Cast the result to ActionResult to allow access to .error
    const result = (banned ? await unbanUser(id) : await banUser(id)) as ActionResult;
    if (result.success) {
  ...
  const handleRoleChange = async (id: string, role: Role) => {
    const result = (await changeUserRole(id, role)) as ActionResult;
    if (result.success) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    const result = (await deleteUser(id)) as ActionResult;
    if (result.success) {
